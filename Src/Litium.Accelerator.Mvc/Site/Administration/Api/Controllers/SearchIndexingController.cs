...
{
    [RoutePrefix("site/administration/api/searchindexing")]
            ...
            where TTemplate : FieldTemplate where TArea : IArea
        {
            ...
            foreach (var item in templates.OfType<TTemplate>())
            {
                ...
                var template = new SearchIndexingModel.Template
                {
                    ...
                    SelectedFields = templateFields.Where(x => selectedFields.Contains(x.FieldId)).ToList(),
                    Fields = templateFields.Where(x => !selectedFields.Contains(x.FieldId)).ToList()
                };
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public IHttpActionResult Put([FromBody] SearchIndexingModel model)
        {
            ...
            foreach (var groupedTemplates in model.GroupedTemplates)
            {
                ...
                {
                    _templateSettingService.SetTemplateIndexingFields(item.AreaType, item.TemplateId, item.SelectedFields.Select(x => x.FieldId).ToList());
                }
            }
