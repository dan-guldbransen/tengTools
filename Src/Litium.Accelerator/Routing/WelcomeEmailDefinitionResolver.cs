...
namespace Litium.Accelerator.Routing
{
    ...
    public class WelcomeEmailDefinitionResolver
    {
        ...
        public WelcomeEmailDefinition Get(WelcomeEmailModel model, string toEmail)
        {
            ...
            if (page == null)
            {
                _logger.LogError($"Cannot find the welcome email page at channel({ChannelModel.SystemId}) and website({ChannelModel.Channel.WebsiteSystemId}). Cannot send email to {toEmail}.");
            }
