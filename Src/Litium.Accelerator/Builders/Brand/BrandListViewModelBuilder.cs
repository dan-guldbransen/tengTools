...
namespace Litium.Accelerator.Builders.Brand
{
    public class BrandListViewModelBuilder : IViewModelBuilder<BrandListViewModel>
    {
        ...
        public virtual BrandListViewModel Build(PageModel pageModel)
        {
            var model = pageModel.MapTo<BrandListViewModel>();
            model.Nodes = GetBrandNodes(model.MaxColumns);
            return model;
        }

        private BrandListViewModel.BrandNode GetBrandNodes(int maxColumns)
        {
            var brandNodes = new BrandListViewModel.BrandNode();

            var allBrandPages = GetNavigationModel();
        ...
        private IEnumerable<ContentLinkModel> GetNavigationModel()
        {
            ...
            var tagTerm = _productSearchService.GetTagTerms(searchQuery, new[] { BrandListViewModel.TagName }).FirstOrDefault();
