...
namespace Litium.Accelerator.Search
{
    public static class SearchQueryExtensions
    {
        ...
        {
            return searchQuery.Tags.Count(x => exceptTag is null || exceptTag != x.Key) > 0
        ...
        public static string GetUrlSort(this SearchQuery searchQuery, string sortBy, Framework.Search.SortDirection sortDirection)
        {
            return searchQuery.GetUrl(query =>
            {
                query.SortBy = sortBy;
        ...
        public static bool IsSortedBy(this SearchQuery searchQuery, string field, Framework.Search.SortDirection direction)
        {
            if (direction == Framework.Search.SortDirection.Descending)
            {
                return Framework.Search.SortDirection.Descending == searchQuery.SortDirection && field.Equals(searchQuery.SortBy, StringComparison.OrdinalIgnoreCase);
            }

            return Framework.Search.SortDirection.Ascending == searchQuery.SortDirection && field.Equals(searchQuery.SortBy, StringComparison.OrdinalIgnoreCase);
        }
    }
}
